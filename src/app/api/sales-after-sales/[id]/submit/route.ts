...
export async function POST(request: Request, context: Context) {
  try {
    optionalSaleAfterSaleNoteSchema.parse(await parseOptionalSalesAfterSaleJson(request));
    const afterSaleCase = await salesAfterSalesService.submit(DEFAULT_OWNER_ID, (await context.params).id);
