...
import { sellerRejectSchema } from "@/server/purchase-after-sales/purchase-after-sales-validation";

type Context = { params: Promise<{ id: string }> };
export async function POST(request: Request, context: Context) {
  try {
    const input = sellerRejectSchema.parse(await parseJsonBody(request));
    const afterSaleCase = await purchaseAfterSalesService.sellerReject(DEFAULT_OWNER_ID, (await context.params).id, input.reason ?? input.note ?? "");
