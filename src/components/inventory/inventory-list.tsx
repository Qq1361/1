...
export function InventoryList(_props: { showHeader?: boolean } = {}) {
  ...
  const [targetLocationMode, setTargetLocationMode] = useState<"MANUAL" | "STANDARD">("MANUAL");
  const [targetManualLocation, setTargetManualLocation] = useState("");
  ...
  function buildOperationPayload() {
    if (operation === "MOVE_LOCATION") {
      return targetLocationMode === "MANUAL"
        ? { locationMode: "MANUAL", warehouseId: targetWarehouseId, storageLocation: targetManualLocation, storageLocationId: null }
        : { locationMode: "STANDARD", warehouseId: targetWarehouseId, storageLocationId: targetLocationId, storageLocation: null };
    }
      ...
      <Dialog key={maintenanceDialog ?? "closed"} open={maintenanceDialog !== null} onOpenChange={(open) => { if (!open) { setMaintenanceDialog(null); setPreview(null); } }}>
        <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-xl">
          ...
          <div className="space-y-4 text-sm">
            ...
            {operation === "MOVE_LOCATION" ? <>{!warehouses.length ? <p className="rounded-md bg-destructive/10 p-3 text-destructive">暂无启用仓库，请先前往 <Link className="underline" href="/inventory/warehouses">仓库管理</Link> 创建或启用仓库。</p> : null}<div className="space-y-2"><Label htmlFor="bulk-target-warehouse">目标仓库</Label><Select value={targetWarehouseId} onValueChange={(value) => { setTargetWarehouseId(value); setTargetLocationId(""); setPreview(null); }}><SelectTrigger id="bulk-target-warehouse"><SelectValue placeholder="选择仓库" /></SelectTrigger><SelectContent>{warehouses.map((warehouse) => <SelectItem key={warehouse.id} value={warehouse.id}>{warehouse.name}</SelectItem>)}</SelectContent></Select></div><div className="space-y-2"><Label htmlFor="bulk-location-mode">库位录入方式</Label><Select value={targetLocationMode} onValueChange={(value) => { setTargetLocationMode(value as "MANUAL" | "STANDARD"); setTargetLocationId(""); setTargetManualLocation(""); setPreview(null); }}><SelectTrigger id="bulk-location-mode"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="MANUAL">手动填写</SelectItem><SelectItem value="STANDARD">标准库位</SelectItem></SelectContent></Select></div>{targetLocationMode === "MANUAL" ? <div className="space-y-2"><Label htmlFor="bulk-target-manual-location">手动库位</Label><Input id="bulk-target-manual-location" value={targetManualLocation} onChange={(event) => { setTargetManualLocation(event.target.value); setPreview(null); }} disabled={!targetWarehouseId} placeholder="例如 货架2-第3层" /></div> : <div className="space-y-2"><Label htmlFor="bulk-target-location">目标库位</Label><Select value={targetLocationId} onValueChange={(value) => { setTargetLocationId(value); setPreview(null); }} disabled={!targetWarehouseId}><SelectTrigger id="bulk-target-location"><SelectValue placeholder="选择库位" /></SelectTrigger><SelectContent>{targetLocations.map((location) => <SelectItem key={location.id} value={location.id}>{location.name}</SelectItem>)}</SelectContent></Select></div>}</> : null}
