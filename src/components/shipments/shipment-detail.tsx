...
import { ArrowLeft, Check, Loader2, Pencil, PackageCheck, Save, Truck, X } from "lucide-react";
...
export function ShipmentDetail({ id }: { id: string }) {
  ...
  function startEditing() {
    ...
    setEditForm({
      ...
      outboundShippingCost: batch.outboundShippingCost || "",
      packagingCost: batch.packagingCost || "",
      otherShipmentCost: batch.otherShipmentCost || "",
      note: batch.note || "",
    });
    setEditing(true);
  }
  ...
  async function confirmShip() {
    if (!shipForm.carrierCode.trim() || !shipForm.trackingNo.trim()) {
      toast.error("请填写快递公司和快递单号"); return;
    }
    setConfirming(true);
    try {
      // First save batch info if needed
      const needSave = shipForm.carrierCode !== (batch?.carrierCode || "") ||
        shipForm.trackingNo !== (batch?.trackingNo || "") ||
        shipForm.note !== (batch?.note || "");
      if (needSave) {
        await fetch(`/api/shipments/${id}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify(shipForm) });
      }
  ...
  const lineActions: Record<string, (l: LineDetail) => LineAction[]> = {
    DRAFT: (l) => [
      { label: l.packedChecked ? "取消核对" : "核对装箱", fn: () => updateLine(l.id, { packedChecked: !l.packedChecked }) },
      { label: "移除", fn: () => { if (confirm("确认从草稿移除该库存？")) updateLine(l.id, {}); fetch(`/api/shipments/lines/${l.id}`, { method: "DELETE" }).then(() => reload()); } },
    ],
    SHIPPED: (l) => [
      { label: "平台已签收", fn: () => updateLine(l.id, { lineStatus: "RECEIVED" }) },
      { label: "平台拒收", fn: () => { const r = prompt("拒收原因："); if (r !== null) updateLine(l.id, { lineStatus: "REJECTED", rejectedReason: r }); } },
      { label: "退回中", fn: () => { const c = prompt("退回快递公司：") || ""; const t = prompt("退回单号：") || ""; if (c || t) updateLine(l.id, { lineStatus: "RETURNING", returnCarrierCode: c || undefined, returnTrackingNo: t || undefined }); } },
    ],
  };

  function getLineActions(l: LineDetail): LineAction[] {
    if (lineActions[l.lineStatus]) return lineActions[l.lineStatus](l);
    if (l.lineStatus === "RECEIVED") {
      const a: LineAction[] = [];
      const isLightning = batch!.defaultPurpose === "DEWU_LIGHTNING_INBOUND" || batch!.defaultPurpose === "NINETY_FIVE_INBOUND";
      const isStandard = batch!.defaultPurpose === "DEWU_STANDARD_FULFILLMENT";
      if (isLightning) { a.push({ label: "入仓成功", fn: () => updateLine(l.id, { lineStatus: "IN_WAREHOUSE" }) }); a.push({ label: "上架/可售", fn: () => updateLine(l.id, { lineStatus: "LISTED" }) }); }
      if (isStandard) { a.push({ label: "鉴别通过/待结算", fn: () => updateLine(l.id, { lineStatus: "IN_WAREHOUSE" }) }); }
      a.push(
        { label: "平台拒收", fn: () => { const r = prompt("拒收原因："); if (r !== null) updateLine(l.id, { lineStatus: "REJECTED", rejectedReason: r }); } },
        { label: "退回中", fn: () => { const c = prompt("退回快递公司：") || ""; const t = prompt("退回单号：") || ""; if (c || t) updateLine(l.id, { lineStatus: "RETURNING", returnCarrierCode: c || undefined, returnTrackingNo: t || undefined }); } },
      );
    ...
    if (l.lineStatus === "IN_WAREHOUSE") {
      const a: LineAction[] = [];
      if (batch!.defaultPurpose === "DEWU_LIGHTNING_INBOUND" || batch!.defaultPurpose === "NINETY_FIVE_INBOUND") a.push({ label: "上架/可售", fn: () => updateLine(l.id, { lineStatus: "LISTED" }) });
      a.push(
        { label: "平台拒收", fn: () => { const r = prompt("拒收原因："); if (r !== null) updateLine(l.id, { lineStatus: "REJECTED", rejectedReason: r }); } },
        { label: "退回中", fn: () => { const c = prompt("退回快递公司：") || ""; const t = prompt("退回单号：") || ""; if (c || t) updateLine(l.id, { lineStatus: "RETURNING", returnCarrierCode: c || undefined, returnTrackingNo: t || undefined }); } },
      );
    ...
    if (l.lineStatus === "LISTED") return [
      { label: "平台拒收", fn: () => { const r = prompt("拒收原因："); if (r !== null) updateLine(l.id, { lineStatus: "REJECTED", rejectedReason: r }); } },
      { label: "退回中", fn: () => { const c = prompt("退回快递公司：") || ""; const t = prompt("退回单号：") || ""; if (c || t) updateLine(l.id, { lineStatus: "RETURNING", returnCarrierCode: c || undefined, returnTrackingNo: t || undefined }); } },
    ];
    if (l.lineStatus === "REJECTED") return [{ label: "退回中", fn: () => { const c = prompt("退回快递公司：") || ""; const t = prompt("退回单号：") || ""; if (c || t) updateLine(l.id, { lineStatus: "RETURNING", returnCarrierCode: c || undefined, returnTrackingNo: t || undefined }); } }];
    if (l.lineStatus === "RETURNING") return [{ label: "已退回本地", fn: () => { const loc = prompt("退回后存放库位：") || ""; updateLine(l.id, { lineStatus: "RETURNED", returnedStorageLocation: loc || undefined }); } }];
    if (l.lineStatus === "RETURNED") return [{ label: "确认重新入库", fn: () => fetch(`/api/shipments/lines/${l.id}/confirm-restocked`, { method: "POST" }).then(r => { if (r.ok) { toast.success("已重新入库"); reload(); } else r.json().then(d => toast.error(d.message)); }).catch(() => toast.error("网络异常")) }];
    return [];
  }
    ...
    <div className="space-y-5">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          {isDraft ? <Button variant="outline" size="sm" onClick={() => { if (confirm("确认取消批次？")) fetch(`/api/shipments/${id}/cancel`, { method: "POST" }).then(r => { if (r.ok) { toast.success("已取消"); reload(); } else r.json().then(d => toast.error(d.message)); }).catch(() => toast.error("网络异常")); }}><X />取消批次</Button> : null}
          {["DRAFT", "SHIPPED"].includes(batch.status) && !isDraft ? <Button variant="outline" size="sm" onClick={() => { if (confirm("确认取消批次？")) fetch(`/api/shipments/${id}/cancel`, { method: "POST" }).then(r => { if (r.ok) { toast.success("已取消"); reload(); } else r.json().then(d => toast.error(d.message)); }).catch(() => toast.error("网络异常")); }}><X />取消批次</Button> : null}
          {batch.status === "SHIPPED" || batch.status === "PARTIALLY_RECEIVED" ? <Button size="sm" variant="outline" onClick={() => fetch(`/api/shipments/${id}/mark-received`, { method: "POST" }).then(r => { if (r.ok) { toast.success("整批已签收"); reload(); } else r.json().then(d => toast.error(d.message)); }).catch(() => toast.error("网络异常"))}><PackageCheck />整批签收</Button> : null}
        </div>
      </div>

      {/* Batch info card with edit mode */}
      <Card>
        ...
        <CardContent>
          {editing ? (
            <div className="grid gap-4 sm:grid-cols-2">
              {isDraft ? (<>
                <div className="space-y-1"><Label className="text-xs">平台</Label>
                  <select className="h-8 w-full rounded-lg border bg-background px-2 text-sm" value={editForm.platform} onChange={e => setEditForm({ ...editForm, platform: e.target.value })}>
                    <option value="DEWU">得物</option><option value="NINETY_FIVE">95分</option><option value="OTHER">其他</option>
                  </select>
                </div>
                <div className="space-y-1"><Label className="text-xs">寄送目的</Label>
                  <select className="h-8 w-full rounded-lg border bg-background px-2 text-sm" value={editForm.defaultPurpose} onChange={e => setEditForm({ ...editForm, defaultPurpose: e.target.value })}>
              ...
              <div className="flex gap-2 sm:col-span-2">
                <Button size="sm" onClick={saveEdit} disabled={saving}>{saving ? <Loader2 className="animate-spin" /> : <Save />}保存</Button>
            ...
            <div className="grid gap-3 text-sm sm:grid-cols-2">
              ...
              <Info label="发往平台运费" text={batch.outboundShippingCost ? `¥${batch.outboundShippingCost}` : "未填写"} />
              <Info label="包材成本" text={batch.packagingCost ? `¥${batch.packagingCost}` : "未填写"} />
        ...
        <Card className="border-primary">
          ...
          <CardContent>
            ...
            <div className="mt-4 flex gap-2">
              <Button size="sm" onClick={confirmShip} disabled={confirming}>{confirming ? <Loader2 className="animate-spin" /> : <Truck />}确认发货</Button>
              <Button size="sm" variant="outline" onClick={() => setShowShipDialog(false)}>取消</Button>
            </div>
          </CardContent>
        </Card>
      ) : null}
      ...
      {batch.actionLogs?.length > 0 ? (
        <Card><CardHeader><CardTitle className="text-base">操作日志</CardTitle></CardHeader>
          <CardContent className="space-y-1 text-xs text-muted-foreground">
            {batch.actionLogs.slice(0, 30).map((log) => (
              <div key={log.id}>{new Date(log.createdAt).toLocaleString("zh-CN")} · {log.actionType}{log.note ? ` · ${log.note}` : ""}</div>
            ))}
          </CardContent>
        </Card>
      ) : null}
