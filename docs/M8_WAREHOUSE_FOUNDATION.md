...
- 待验货列表的批量入口先打开资料弹窗。用户可将仓库、库位、成色、计划出售方式和公共备注显式应用到全部商品，再做逐件覆盖；确认前只存在浏览器草稿，不写数据库。
- 每件入库必须使用当前 owner 的启用仓库和该仓库的启用库位。服务端在 Serializable 事务内重新校验所有关联，任何一项过期、跨 owner 或不匹配都会让全批回滚。
