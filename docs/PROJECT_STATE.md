...
- `pnpm verify:m5-purchase-items` 已扩展并通过 98 项有意义检查。M5-A2 保持当前业务边界，不新增 schema 或 migration。

## M5-A1 采购订单商品明细维护（已完成）

- 已支持采购订单创建后的商品明细添加、编辑和删除；最后一条明细不可删除。
...
- 创建页和订单详情维护入口共用严格的 Decimal 字符串校验；服务端在分摊、库存、验货、采购售后或采购退款等下游流程后锁定明细维护，单独付款状态不构成锁定。
