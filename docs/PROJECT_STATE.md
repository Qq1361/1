...
- 当前库存到期日以 `Asia/Shanghai` 日期口径统一分为已过期、30 天内、90 天内和 180 天内四个互斥风险；空到期日和 180 天以上不进入风险。
- 工作台、库存列表、日报与飞书日报共用同一只读规则。仅支持、归属为 `OWNED` 且非 `SOLD` 的资产参与；不改变库存状态、成本、采购、销售、售后、物流或 `SOLD` 规则。
- 新增 `GET /api/inventory/expiry-risk`、库存 `expiryRisk` 筛选和 `pnpm verify:m7-expiry-reminders`；没有 Prisma schema 或 migration 变更。

## M7-A1 采购到库存保质期追踪（已完成）
...
- 待验货 `/inspections` 支持服务端按采购订单号、商品、SKU 和卖家昵称搜索；结果分页 total 与列表共用同一 owner/待验货条件，列表直接显示卖家昵称。
...
- 批量 SKU/色号已收口为独立服务端逐件预览与指纹确认：默认只补空 SKU，已有 SKU 保持不变；跨商品和包含已售库存档案都必须明确确认。SKU 仅修正库存档案，不回写销售 SKU 快照，也不写状态、成本或销售事实。
- 已售、非在库、销售关联、平台寄送/退回或售后关联库存会锁定整批；不修改状态、成本、价格、采购、销售、售后、退款或物流事实。

## M8 第四阶段：正式仓库下的手动库位（完成）

- `InventoryItem.warehouseId` 继续表示正式仓库；`storageLocationId` 表示标准库位；`storageLocation` 表示手动自由文本。请求必须显式带 `locationMode`，不会从旧字段猜测写入意图。
- 批量验货和批量移动默认支持手动库位：手动模式保存仓库与自由文本、清空标准库位；标准模式保存仓库与标准库位、清空自由文本。两种模式都校验 owner、启用状态和输入格式。
- 库存展示优先显示“仓库 / 标准库位”或“仓库 / 手动库位”；历史自由文本和仅仓库记录有兼容显示。变更预览与 `InventoryItemActionLog` 保存完整位置前后快照。
- `pnpm verify:m8-manual-storage-location` 覆盖手动入库、混合批量验货、标准/手动三向迁移、原子校验、展示与审计边界；不新增状态、成本、销售、寄送、售后或 `SOLD` 写入。
