...
- 到期日按 `Asia/Shanghai` 的日期比较：`expiryDate < today` 为已过期；其余依次为 30、90、180 天内。边界日期只进入一个风险档位。
- `expiryDate = null` 和超过 180 天的库存不属于效期风险。`SOLD`、非 `OWNED` 资产和当前不支持状态不参与。
- 风险提醒为只读展示和跳转条件。它不会自动下架、改变 `itemStatus`、成本、归属、采购/销售/售后事实或写入 `SOLD`；`PLATFORM_LISTED` 也不会被误当作已售。
- 库存列表、工作台、日报和飞书日报必须复用共享规则，不能自行使用客户端本地时钟重新计算权威风险。
