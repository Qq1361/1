...
商品维护遇到活动成本分摊草稿时，采购详情会显示草稿锁定提示、查看入口和“放弃草稿并编辑商品”操作。该操作仅处理尚未确认的 `DRAFT`：系统以当前 `allocationVersion` 为并发条件，将订单还原为 `UNALLOCATED` 并清空临时 `allocatedTotalCost`，同时记录采购操作日志。

已确认分摊不能通过此入口撤销。放弃草稿不会删除商品、改变采购实付或库存成本，也不会改变验货、库存、销售或售后；其余商品维护锁定继续生效。
