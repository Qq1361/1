...
- [x] `/purchases/new` 提供普通录入和批量录入，默认保持普通录入行为。
- [x] 批量行固定数量为 1，支持 1 至 50 行；相同商品名和 SKU 不会合并。
- [x] 批量行可独立保存 SKU、参考成交总额、生产日期、保质期月数和到期日期。
- [x] 新增空白、复制当前、复制首行和删除未提交行不会产生数据库记录；最后一行不能删除。
- [x] 批量创建为原子 transaction；无效行、超过 50 行、混合模式、客户端 quantity/ownerId 或未知字段均被拒绝且无部分写入。
- [x] 批量创建不修改订单实付、运费、成本、退款、验货、库存、销售、日报或 `SOLD`。
- [x] `pnpm verify:m7-purchase-create-batch` 通过真实 HTTP 与精确夹具清理验证。

## M7-A2 库存临期与过期提醒

- [x] `Asia/Shanghai` 日期边界将已过期、30 天内、90 天内、180 天内互斥分类；空日期和 180 天以上不告警。
- [x] 工作台风险卡、库存筛选、日报和飞书格式化复用同一只读聚合，并显示库位摘要。
- [x] `SOLD`、非 `OWNED` 和不支持状态被排除；提醒不写库存状态、不产生下架、采购、销售、售后或 `SOLD`。
- [x] `pnpm verify:m7-expiry-reminders` 覆盖边界、过滤、日报一致性、只读快照和精确夹具清理。

## M7-A1 保质期追踪
- [x] 采购明细支持可选生产日期、保质期月数和到期日期，采用 `DATE` / `YYYY-MM-DD` date-only 口径。
- [x] 生产日期加月数按月末夹位计算；手工到期日优先，日期先后与月数范围由服务端和数据库约束保护。
- [x] 新建采购、单条添加、批量添加、复制行和未锁定编辑均支持独立保质期字段；空白行不继承。
- [x] 单件和批量验货复用同一完成核心并复制库存快照；相同 SKU 的不同到期日保持独立。
- [x] 库存列表和详情显示快照；无提醒、自动状态、成本、退款、销售、售后、物流或 `SOLD` 写入变化。
- [x] `pnpm verify:m7-shelf-life` 覆盖专项规则与精确夹具清理；M7-A2 未开始。
...
- [x] M6-A0 已冻结 M6-A1～M6-A4 的最小实施拆分。
- [x] 第 25 条纯加法 migration 创建通用 shipment/event 模型、枚举、索引、外键和 CHECK，不改旧 `LogisticsEvent`。
- [x] Provider 注册表当前只有确定性本地 `MOCK`，未知 Provider 稳定拒绝，不访问公网。
- [x] 统一状态映射、运单号标准化、Provider 响应校验和轨迹 `dedupeKey` 均有独立规则与测试。
- [x] 五种 businessType 分别绑定真实业务模型，所有注册、查询和同步均按 owner 隔离。
- [x] 首次同步写入多事件，重复同步幂等，后续轨迹只追加新事件，首次 `deliveredAt` 保留。
- [x] Provider 失败保留旧轨迹和当前状态，可重试失败记录次数和安全错误码。
- [x] 通用 Service 只写两张新物流表，不改采购、寄送、退回、售后、库存、验货、退款、销售、日报或 `SOLD`。
- [x] M6-A1 阶段没有公开 API、页面、任务、Webhook、真实 Provider 凭证或真实物流请求；M6-A2 已在独立章节完成手动查询能力验收。

## M5-A3 签收后移除误录采购商品

- [x] 订单详情对人工签收后的未处理占位 Inspection 提供“移除误录商品”和原因选择；普通删除路径保持不变。
- [x] `PurchaseOrderActionLog` 使用独立 migration、owner/order/createdAt 索引和最小快照字段，不复用采购售后日志，不回填历史。
- [x] Serializable 事务显式处理占位 Inspection；已有验货结果、备注、附件、库存、成本、售后或退款时拒绝，不依赖级联删除。
- [x] `pnpm verify:m5a3-entry-error-removal` 通过 47 项检查，覆盖 25 改 24、审计日志、回滚、并发、跨 owner、付款/签收/SOLD 不变。
...
- [x] `pnpm verify:m5-purchase-items` 通过 121 项有意义检查；M5-A2 未修改 schema、migration、状态机或 `SOLD` 写入规则。

## M5-A1 采购订单商品明细维护

- [x] 创建采购订单时可选填写“商品参考成交总额（可选）”，未填写显示“未填写”。
- [x] 订单创建后可添加、编辑和删除商品明细；删除最后一条明细返回冲突错误。
- [x] 删除资格按目标商品行返回；已付款、运单和物流提醒时间不锁定，成本分摊、目标商品验货、库存、采购售后或采购退款仍锁定；并发删除不会留下空订单。
- [x] 创建页和详情页维护使用相同的字段和严格金额校验；非法金额和未知字段返回 400。
- [x] 付款本身不锁定明细；分摊、库存、验货、采购售后和采购退款存在时由服务端锁定。
- [x] 参考成交总额不改变订单实付、成本分摊、库存成本、采购退款或日报统计。
- [x] `pnpm verify:m5-purchase-items` 通过 121 项 M5-A1/M5-A2 检查，验证数据按精确 ID 清理。
...
- [ ] 保存按钮不卡死
- [ ] 加载失败显示错误卡片（不空白）
- [x] 未确认草稿显示“一键平均分摊”，按实际商品总件数而非商品行数计算，并展示待分摊总金额、行数、总件数、平均单件成本、当前分摊合计与差额。
- [x] 平均结果仅填入页面草稿；非空未确认输入会先确认覆盖，取消后保留原值，不自动保存或确认。
- [x] 使用 Decimal/整数分精确分摊，余数按商品创建时间和 ID 的稳定顺序分到具体件；数量大于 1 的行保存对应行总成本。
- [x] 预览后的订单金额或商品数量变化会拒绝旧版本保存；已确认分摊不显示重新平均入口。
- [x] `pnpm verify:m1-equal-cost-allocation` 覆盖按件数、余数、预览不落库、冲突、已确认与 owner 隔离。
- [x] 活动 `DRAFT` 成本分摊草稿锁定采购商品维护；采购详情提供查看草稿与放弃草稿入口，并要求二次确认。
- [x] 放弃草稿使用当前 `allocationVersion` 并发保护，只清除临时分摊状态和值，写入采购操作日志；已确认分摊、跨 owner、过期版本和无草稿请求均被拒绝。
- [x] 放弃草稿后单件添加、批量添加、编辑与符合既有资格的删除恢复；最后一件商品和下游库存、验货、采购售后、退款等锁定不被绕过。
- [x] `pnpm verify:m1-cost-allocation-draft-discard` 覆盖 HTTP、数据边界、审计、并发和 1440px/390px 浏览器验收。

### 待验货 `/inspections`

- [ ] 待验货记录列表正确
- [x] 可按采购订单号、商品、SKU 或卖家昵称进行服务端搜索；卖家昵称支持部分匹配，英文不区分大小写。
- [x] 待验货卡片直接显示卖家昵称；空昵称显示“卖家：—”。
- [x] 搜索、分页和 total 使用同一 owner/待验货条件；变更关键词、分页或每页数量会清空批量选择。
- [ ] 可点击进入验货向导
...
- [x] 快递鸟 1002 Adapter、固定 endpoint、签名、超时、响应大小、JSON 校验和本地注入 Transport 已实现。
- [x] 状态、北京时间轨迹排序、未知状态回退和手机号/座机脱敏有单元验证。
- [x] Provider 状态、采购物流读取、幂等注册和手动同步 API 受现有访问认证与 owner 隔离保护。
- [x] 采购详情显示未配置提示、已保存轨迹、同步错误和签收后人工验货提示；页面加载不调用 Provider。
...
- [x] 点击批量入口先打开“批量验货并入库”资料弹窗；用户确认前不保存、不通过、不创建库存。
- [x] 每件必须选择启用且同 owner 的仓库、匹配且启用的库位与成色；仓库变更会清除不匹配库位，saleMode 为空或 NONE 不触发销售流程。
- [x] 公共资料必须点击“应用到全部”才覆盖草稿，逐件覆盖优先；公共/单件备注和保质期实物修正均以追加审计块保留原有验货备注。
- [x] 批量入库保质期使用 `YYYY-MM-DD` date-only 校验，手工到期日优先；与采购快照不一致时服务端要求修正依据。
- [x] `pnpm verify:m2b-batch-inspection-details` 覆盖独立库存、结构化库位、货架期修正、原子回滚、重复/上限与并发冲突。
- [x] M8 批量库存管理：单选、当前页全选、筛选结果全选、跨页选择、筛选变化清空选择与 200 件上限。
- [x] M8 批量库存管理：仓库库位匹配、成色/跨商品确认、计划出售方式、保质期预览和原子提交。
- [x] M8 手动库位：批量验货和库存批量移动均可在启用正式仓库下显式选择“手动填写”或“标准库位”；两种模式互斥，预览和操作日志保存仓库、模式、标准库位和自由文本快照。
- [x] M8 批量 SKU/色号：服务端逐件预览、默认补空、跨商品与已售档案显式确认、确认指纹与并发回滚；不回写销售 SKU 快照。
- [x] M8 批量库存管理：每件审计日志、锁定项整批回滚，且不改状态/成本/销售/售后/退款/物流。
