...
>
> 日期：2026-07-18
> 实施状态：尚未接入任何真实物流供应商，尚未购买、注册或配置物流 API
...
M6-A0 只冻结设计，不实现 Service、API、页面、任务、供应商适配器或数据库模型。
...
- 完成账号资格和套餐验证后选择快递100或快递鸟。
- 实现真实查询 Adapter、严格配置、手动刷新 API、物流轨迹 DTO 和页面。
- 真实供应商验收仅使用专门测试运单；自动测试仍用本地 mock。
...
- 物流事实与 ERP 业务状态必须分层。
- M6-A0 标记为 `COMPLETED / FROZEN`；M6-A1 尚未开始。
