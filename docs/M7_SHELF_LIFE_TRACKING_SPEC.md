...
## M7-A2 Inventory expiry reminders (completed)

- M7-A2 is a read-only reminder layer over the existing `InventoryItem.expiryDate` snapshot. It adds no schema or migration and does not change shelf-life entry, procurement cost, sales, after-sales, logistics, ownership, or inventory status.
- The business date is evaluated as a date-only `Asia/Shanghai` calendar date. The exclusive bands are `EXPIRED` (`expiryDate < today`), `WITHIN_30_DAYS`, `WITHIN_90_DAYS`, and `WITHIN_180_DAYS`. A null expiry date and an expiry date more than 180 days away are not risks.
- Only supported, owned, non-`SOLD` inventory assets participate. `SOLD` and non-owned upstream-return assets are excluded; platform-listing status is not treated as sold or changed by this feature.
- The shared expiry rule module drives the inventory API, inventory list filter, dashboard summary, daily report, and Feishu report formatter. The feature is observational only: it never writes `SOLD`, changes `itemStatus`, delists items, creates after-sales, or disposes of inventory.
- `pnpm verify:m7-expiry-reminders` provides isolated fixture coverage for dates, boundaries, exclusions, summaries, list filtering, daily-report consistency, and read-only side-effect boundaries.
